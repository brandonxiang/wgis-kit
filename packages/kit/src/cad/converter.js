 ...
 * @param {{ offset?: number[], scale?: number, rotation?: number }} options
...
function convertPolyline(entity, options) {
  ...
  const coordinates = vertices.map(/** @param {any} v */ (v) => transformPoint([v.x, v.y], options))
...
function convertLwpolyline(entity, options) {
  ...
  const coordinates = points.map(/** @param {any} p */ (p) => transformPoint([p.x, p.y], options))
...
function convertCircle(entity, options) {
  ...
  const coordinates = circlePoints.map(
    /** @param {[number, number]} p */ (p) => [
...
function convertArc(entity, options) {
  ...
  const coordinates = arcPoints.map(
    /** @param {[number, number]} p */ (p) => [
      center[0] + p[0] - center[0],
      center[1] + p[1] - center[1],
    ],
...
function convertHatch(entity, options) {
  ...
  const coordinates = [
    boundaryPoints.map(/** @param {any} p */ (p) => transformPoint([p.x, p.y], options)),
  ]
...
function convertEllipse(entity, options) {
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    points.push([center[0] + semiMajor * Math.cos(angle), center[1] + semiMinor * Math.sin(angle)])
  }
...
function convertSpline(entity, options) {
  ...
  const coordinates = controlPoints.map(
    /** @param {any} p */ (p) => transformPoint([p.x, p.y], options),
  )
...
function convertEntity(entity, options) {
  ...
  /** @type {Record<string, (entity: any, options: any) => import('geojson').Feature | null>} */
  const converters = {
    LINE: convertLine,
 ...
 * @param {any[]} entities - Array of CAD entities
 * @param {any} options - Conversion options
 * @returns {import('geojson').FeatureCollection}
 */
