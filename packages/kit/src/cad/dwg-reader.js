 ...
 * @returns {{ entities: any[], layers: string[], warning: string }}
 */
...
export function getDwgVersion(buffer) {
  ...
  return DWG_VERSIONS[signature] || "unknown"
}

/**
 * Suggest conversion approach for DWG
 * @returns {{ recommended: string, alternatives: string[] }}
 */
export function getDwgSuggestions() {
  return {
    recommended:
      "Convert DWG to DXF using:\n" +
