...
const transformFunctions = {
  ...
  "wgs84-bd09": (/** @type {number} */ lng, /** @type {number} */ lat) => {
    const gcj = wgs84togcj02(lng, lat)
    return gcj02tobd09(gcj[0], gcj[1])
  },
  "bd09-wgs84": (/** @type {number} */ lng, /** @type {number} */ lat) => {
 ...
 * @param {(lng: number, lat: number) => [number, number]} transformFn
 * @returns {number[]}
 */
...
function transformCoords(coords, transformFn) {
  ...
  return coords.map(/** @param {any} c */ (c) => transformCoords(c, transformFn))
}
...
function transformGeometry(geometry, transformFn) {
  ...
  if (geometry.type === "GeometryCollection") {
    return {
      ...
      geometries: geometry.geometries.map((g) => transformGeometry(g, transformFn)),
    }
  }

  return {
    ...geometry,
    coordinates: transformCoords(geometry.coordinates, transformFn),
  }
}

/**
 * 转换 GeoJSON 中所有坐标
 * @param {import('geojson').FeatureCollection | import('geojson').Feature | import('geojson').Geometry} geojson
 * @param {'wgs84-gcj02' | 'gcj02-wgs84' | 'gcj02-bd09' | 'bd09-gcj02' | 'wgs84-bd09' | 'bd09-wgs84'} transform
 * @returns {import('geojson').FeatureCollection | import('geojson').Feature | import('geojson').Geometry}
 */
  ...
  if (geojson.type === "FeatureCollection") {
    return {
      ...
      features: geojson.features.map(
        /** @param {any} f */ (f) =>
          /** @type {import('geojson').Feature} */ (transformGeoJSON(f, transform)),
      ),
    }
  }
